...
class RayEncoder(nn.Module):
    """Ray encoder with multi-query, multi-head attention.

    - num_queries (M): number of queries
    - num_heads (H): attention heads; requires d_model % H == 0
    - learnable_queries: when True, query vectors are learnable parameters
    """
    ...
    def __init__(self, vec_dim: int, hidden: int = 64, d_model: int = 128, *, num_queries: int = 1, num_heads: int = 1, learnable_queries: bool = True):
        ...
        # Pose features: sin_ref, cos_ref, prev_vx/lim, prev_omega/lim, dprev_vx/(2*lim), dprev_omega/(2*omega_max), dist_to_task/patch_meters
        pose_dim = 7
